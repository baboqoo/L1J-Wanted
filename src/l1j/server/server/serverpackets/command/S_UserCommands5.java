...
public class S_UserCommands5 extends ServerBasePacket {
	...
	private int Rank(L1PcInstance pc, int number) {
		...
		try {
			...
			switch (number) {
			case 1:
				pstm = con.prepareStatement("SELECT char_name FROM characters WHERE AccessLevel = 0 order by PC_Death desc limit 10");
				break;
			default:
				pstm = con.prepareStatement("SELECT char_name FROM characters WHERE AccessLevel = 0 order by PC_Death desc limit 10");
				break;
			...
			if (number == 9) {
				...
				sql = (new StringBuilder(String.valueOf(sql))).append(" And AccessLevel = 0 order by PC_Death desc").toString();
