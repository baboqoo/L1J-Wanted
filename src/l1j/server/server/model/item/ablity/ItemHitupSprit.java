...
public class ItemHitupSprit implements ItemAbility {// 정령 적중
	private static class newInstance {
		public static final ItemAbility INSTANCE = new ItemHitupSprit();
	}
	public static ItemAbility getInstance(){
		return newInstance.INSTANCE;
	}
	private ItemHitupSprit(){}

	@Override
	public Object info(l1j.server.server.model.Instance.L1ItemInstance item, l1j.server.server.model.Instance.L1PcInstance pc){
		int hit = item.getItem().getHitupSpirit();
		L1EnchantAblity enchantAblity = item.getEnchantAblity();
		if (enchantAblity != null) {
			int value = enchantAblity.getHitupSpirit();
			if (value != 0) {
				hit += value;
			}
		}

		if (pc != null && (pc.getType() == 2 || pc.getType() == 4)) {
