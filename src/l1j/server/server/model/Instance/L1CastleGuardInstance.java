...
public class L1CastleGuardInstance extends L1NpcInstance {
	...
	public boolean noTarget() {
		if (getLocation().getTileLineDistance(new Point(getHomeX(), getHomeY())) > 0) {
			...
			if (getMoveState() != null && getSpawn() != null && getMoveState().getHeading() != getSpawn().getHeading()) {
	...
	public void onTalkAction(L1PcInstance player) {
		...
		if (talking != null) {
			if (npcid == 70549 || npcid == 70985 || npcid == 70656) { 
				...
				if (StringUtil.isNullOrEmpty(clan_name)) {
					clan_name = "$16554"; // 검은 기사단(켄트)
				}
				if (StringUtil.isNullOrEmpty(pri_name)) {
					pri_name = "$16533"; // 켄 라우헬(켄트)
				}
				//htmlid = "ktguard6";
				//htmldata = new String[] { getName(), clan_name, pri_name };
				htmlid = "mercenary1";
				htmldata = new String[] { clan_name, pri_name };
			} else if (npcid == 60560) { 
				for (L1Clan clan : L1World.getInstance().getAllClans()) {
			...
			if (htmlid != null) { 
				...
				player.sendPackets(new S_NPCTalkReturn(talking, objid, player.getAlignment() < -1000 ? 2 : 1), true);
			}
		}
	}
