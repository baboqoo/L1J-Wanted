...
public class PrivateWarehouse extends Warehouse {
	...
	public synchronized void insertItem(L1ItemInstance item) {
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement("INSERT INTO character_warehouse SET id = ?, account_name = ?, item_id = ?, item_name = ?, count = ?, is_equipped=0, enchantlvl = ?, is_id = ?, durability = ?, charge_count = ?, remaining_time = ?, last_used = ?, attr_enchantlvl = ?, bless = ?, special_enchant = ?, doll_ablity = ?, package=?");
	...
	public synchronized void deleteItem2(L1ItemInstance item) {
		Connection con = null;
		PreparedStatement pstm = null;
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
		...
		int index = 0;
		boolean ch = false;
		for (L1ItemInstance itemtemp : _items) {
			if (itemtemp.getId() == item.getId()) {
				index = _items.indexOf(itemtemp);
				ch = true;
				break;
			}
		}
