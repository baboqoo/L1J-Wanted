...
public class ShadowStep extends L1SkillActionHandler {
	...
	public int start(L1Character attacker, L1Character cha, int time, L1Magic magic) {
		...
		if (!(attacker.getX() == targetX && attacker.getY() == targetY)) {
			...
			if (!attacker.isPassiveStatus(L1PassiveId.SHADOW_STEP_CHASER) && !attacker.getMap().isPassable(targetX, targetY)) {
				return 0;// 쉐도우 스텝:체이서 통과
			}
		}
	...
	public L1SkillActionHandler copyInstance() {
		return new ShadowStep().setValue(_skillId, _skill);
	}
