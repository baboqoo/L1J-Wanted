...
public class PowerGrip extends L1SkillActionHandler {

	@Override
	public int start(L1Character attacker, L1Character cha, int time, L1Magic magic) {
		int changeBuffDuration = CommonUtil.randomIntChoice(POWER_GRIP_ARRAY);
		...
		L1EffectSpawn.getInstance().spawnEffect(9415, changeBuffDuration, cha.getX(),cha.getY(), cha.getMapId());
		if (cha instanceof L1PcInstance) {
			L1PcInstance pc = (L1PcInstance) cha;
			pc.sendPackets(S_Paralysis.GRIP_ON);
	...
	public void stop(L1Character cha) {
		if (cha instanceof L1PcInstance) {
			L1PcInstance pc = (L1PcInstance) cha;
			pc.sendPackets(S_Paralysis.GRIP_OFF);
	...
	public L1SkillActionHandler copyInstance() {
		return new PowerGrip().setValue(_skillId, _skill);
	}
