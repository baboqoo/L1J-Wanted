...
public class ShockStun extends L1SkillActionHandler {
	...
	public int start(L1Character attacker, L1Character cha, int time, L1Magic magic) {
		...
		if (diffLevel <= -3) {
			stunTimeArray = SHOCK_STUN_ARRAY_1;
		} else if (diffLevel >= -2 && diffLevel <= 2) {
			stunTimeArray = SHOCK_STUN_ARRAY_2;
		} else if (diffLevel >= 3 && diffLevel <= 5) {
			stunTimeArray = SHOCK_STUN_ARRAY_3;
		} else if (diffLevel >= 6) {
			stunTimeArray = SHOCK_STUN_ARRAY_4;
		}
		...
		L1EffectSpawn.getInstance().spawnEffect(81162, changeBuffDuration, cha.getX(), cha.getY(), cha.getMapId());
	...
	public L1SkillActionHandler copyInstance() {
		return new ShockStun().setValue(_skillId, _skill);
	}
