...
public class Base64 {
	...
	private final static String PREFERRED_ENCODING = CharsetUtil.UTF_8_STR;
	...
	public static String encodeBytes(byte[] source, int off, int len,
		...
		// Else, don't compress. Better not to use streams at all then.
			...
			int len43 = (len << 0x00000002) / 3;
			byte[] outBuff = new byte[(len43) // Main 4:3
					+ ((len & 2) > 0 ? 4 : 0) // Account for padding
	...
	private static int decode4to3(byte[] source, int srcOffset,
		...
		// Example: DkLE
			...
			try {
				...
				System.out.println(StringUtil.EmptyString + source[srcOffset] + ": "
						+ (DECODABET[source[srcOffset]]));
				System.out.println(StringUtil.EmptyString + source[srcOffset + 1] + ": "
						+ (DECODABET[source[srcOffset + 1]]));
				System.out.println(StringUtil.EmptyString + source[srcOffset + 2] + ": "
						+ (DECODABET[source[srcOffset + 2]]));
				System.out.println(StringUtil.EmptyString + source[srcOffset + 3] + ": "
	 ...
	 * @throws Exception
	 */
	public static String decryptToKey(String text, String key) throws Exception {// 복호화
		try{
			...
			byte[] result	=	cipher.doFinal(java.util.Base64.getDecoder().decode(text));
			return new String(result, CharsetUtil.UTF_8_STR);
	 ...
	 * @throws Exception
	 */
	public static String encryptToKey(String text, String key) throws Exception {// 복호화
		try{
			Cipher cipher	=	Cipher.getInstance("AES/CBC/PKCS5Padding");
			byte[] keyBytes	=	new byte[16];
			byte[] b		=	key.getBytes(CharsetUtil.UTF_8_STR);
			...
			byte[] result	=	cipher.doFinal(text.getBytes(CharsetUtil.UTF_8_STR)); //java.util.Base64.getDecoder().decode(
			return new String(result, CharsetUtil.UTF_8_STR);
		}catch(IllegalArgumentException e){
			return null;
		}
	 ...
	 * @throws Exception
	 */
	public static String encryptToBase64(String text, String key) throws Exception {// 복호화
		try{
			Cipher cipher	=	Cipher.getInstance("AES/CBC/PKCS5Padding");
			byte[] keyBytes	=	new byte[16];
			byte[] b		=	key.getBytes(CharsetUtil.UTF_8_STR);
			...
			cipher.init(Cipher.ENCRYPT_MODE, keySpec, ivSpec);
			byte[] result	= java.util.Base64.getEncoder().encode(cipher.doFinal(text.getBytes(CharsetUtil.UTF_8_STR)));
