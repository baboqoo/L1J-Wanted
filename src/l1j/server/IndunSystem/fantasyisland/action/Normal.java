 ...
 * @author LinOffice
 */
public class Normal extends FantasylslandHandler {
	public Normal(L1PcInstance pc, short mapId, FantasylslandType raidType){
	...
	protected void createSpwan() throws Exception {
		...
		BasicNpcList = _util.spawn(_map, 0, true);// 유니콘 스폰
		setting();
		NpcList = _util.spawn(_map, 1, true);// 봉인술사, 경비병 스폰
		Thread.sleep(1000);
		if (!isValidataion()) {
			return;
		}
	...
	protected void firstStep() throws Exception {
		stage = SECOND_STEP;
		_util.spawnDelay(_map, 2, 1000);
		Thread.sleep(4000);
		if (!isValidataion()) {
			return;
		}
		_util.spawnDelay(_map, 3, 1000);
		Thread.sleep(4000);
		if (!isValidataion()) {
			return;
		}
		_util.spawnDelay(_map, 4, 1000);
		Thread.sleep(4000);
		if (!isValidataion()) {
			return;
		}
		_util.spawnDelay(_map, 5, 1000);
	}
	...
	protected void secondStep() throws Exception {
		...
		_util.spawnDelay(_map, 6, 800);
		Thread.sleep(4000);
		if (!isValidataion()) {
			return;
		}
		_util.spawnDelay(_map, 7, 800);
		Thread.sleep(4000);
		if (!isValidataion()) {
			return;
		}
	...
	protected void thirdStep() throws Exception {
		...
		_util.spawnDelay(_map, 8, 600);
		Thread.sleep(3000);
		if (!isValidataion()) {
			return;
		}
		_util.spawnDelay(_map, 9, 600);
		Thread.sleep(3000);
		if (!isValidataion()) {
			return;
		}
		_util.spawnDelay(_map, 10, 600);
		Thread.sleep(20000);
		if (!isValidataion()) {
			return;
		}
	...
	public void raidStart() {
		...
		System.out.println("■■■■■■■■■■ Unicorn Temple (NORMAL) Start ■■■■■■■■■■ MAP - " + _map);
	}

	@Override
	protected void raidClose() {
		stage = CLOSE;
		System.out.println("■■■■■■■■■■ Unicorn Temple (NORMAL) Ends ■■■■■■■■■■ MAP - " + _map);
		dispose();
	}
