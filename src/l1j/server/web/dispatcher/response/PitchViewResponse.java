...
public class PitchViewResponse extends HttpResponseModel {
	...
	public PitchViewResponse() {}
	private PitchViewResponse(HttpRequestModel request, DispatcherModel model) {
		super(request, model);
	}
	...
	public HttpResponse get_response() throws Exception {
		...
		PitchVO cur		= PitchDAO.getBoard(boardNum);
		PitchVO pre		= PitchDAO.getBoard(boardNum - 1);
		PitchVO next	= PitchDAO.getBoard(boardNum + 1);
		StringBuilder sb = new StringBuilder();
		if (pre != null) {
			//sb.append("<p class=\"prev\"><span>이전글</span><a href=\"javascript:urlform(\'").append(pre.getRownum()).append("\', \'post\', \'/pitch/view\');\">").append(pre.getTitle()).append("</a></p>");
			sb.append("<p class=\"prev\"><span>Previous Article</span><a href=\"javascript:urlform(\'").append(pre.getRownum()).append("\', \'post\', \'/pitch/view\');\">").append(pre.getTitle()).append("</a></p>");
		}
		params.add(new KeyValuePair<String, String>("{PRE}",				sb.toString()));
		sb.setLength(0);
		
		if (next != null) {
			//sb.append("<p class=\"next\"><span>다음글</span><a href=\"javascript:urlform(\'").append(next.getRownum()).append("\', \'post\', \'/pitch/view\');\">").append(next.getTitle()).append("</a></p>");
			sb.append("<p class=\"next\"><span>Next Article</span><a href=\"javascript:urlform(\'").append(next.getRownum()).append("\', \'post\', \'/pitch/view\');\">").append(next.getTitle()).append("</a></p>");
		}
		...
		if (account != null && (account.isGm() || (player != null && player.getName().equals(cur.getName())))) {
			sb.append("<button class=\"co-btn co-btn-modify\" onClick=\"javascript:urlform(\'").append(cur.getRownum()).append("\', \'post\', \'/pitch/modify\');\">Edit</button>");
		...
		sb.append("<div class=\"comment-form-textarea\"><textarea class=\"contentWrite\" name=\"content\" placeholder=\"").append(account != null ? "Enter your comment." : "Login to leave a comment.").append("\" onkeyup=\"commentLengthCheck(this);\"></textarea></div>");
		...
		if (cur.getAnswerList() != null) {
			for (PitchCommentVO comment : cur.getAnswerList()) {
	...
	public HttpResponseModel copyInstance(HttpRequestModel request, DispatcherModel model) throws Exception {
		return new PitchViewResponse(request, model);
	}
}
