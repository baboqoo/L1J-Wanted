$(function() {
	...
	listStyleBtn.on('click', function(event) {
		if (boardList.hasClass('board-list-card')) {
			viewMode('list');
			setLocal('board_free_viewMode', 'list');
		}
	});
	...
	cardStyleBtn.on('click', function(event) {
		if (boardList.hasClass('board-list-default')) {
			viewMode('card');
			setLocal('board_free_viewMode', 'card');
		}
	});
...
function reportBtn(obj){
	...
	//reportModalOpen($(obj), '게시글_ID_' + $(obj).attr('data-boardid'));
	reportModalOpen($(obj), 'Post_ID_' + $(obj).attr('data-boardid'));
}
...
function commentReportBtn(obj){
	...
	//reportModalOpen($(obj), '게시글_ID_' + $(obj).attr('data-boardid') + '_코멘트_ID_' + $(obj).attr('data-commentid'));
	reportModalOpen($(obj), 'Post_ID_' + $(obj).attr('data-boardid') + '_Comment_ID_' + $(obj).attr('data-commentid'));
}

function reportTradeBtn(obj){
	if (account == undefined) {
		//popupShow('로그인 후 이용하실 수 있습니다.', '<span class="type2"><a href="javascript:popupClose();" class="close">Close</a></span>', null);
		popupShow('Available after login.', '<span class="type2"><a href="javascript:popupClose();" class="close">Close</a></span>', null);
		return;
	}
	//reportModalOpen($(obj), '거래소_ID_' + $(obj).attr('data-boardid'));
	reportModalOpen($(obj), 'Trade_ID_' + $(obj).attr('data-boardid'));
}
...
function likeBtn(obj){
	...
	boardLikeBtn($(obj), '/define/board/like', {id : $(obj).attr('data-id')});
}
...
function commentLikeBtn(obj){
	...
	boardLikeBtn($(obj), '/define/board/commentLike', {id : $(obj).attr('data-commentid'), boardnum : $(obj).attr('data-boardnum')});
}
...
function createBoardHtml(element){
	...
	return '<li class="board-items" onClick=\'urlform(\"' + element.rownum + '\", "post", "/board/view");\'>' +
				...
				'<div class="comment">' +
				'<a href=\'javascript:urlform(\"' + element.rownum + '\", "post", "/board/view");\'>' +
				...
				'<div class="board-type-card" style=\"' + cardtype + '\">' +
					(element.mainImg != undefined ? '<div class="board-items-thumb"><a href="javascript:urlform(\"' + element.rownum + '\", "post", "/board/view");"><img src=\"' + element.mainImg + '\" alt=""></a></div>' : '') +
					'<div class="board-items-contents">' +
						'<div class="title">' +
							'<a href=\'javascript:urlform(\"' + element.rownum + '\", "post", "/board/view");\'>' + element.title +
							(dateterm===0 ? '&nbsp<i class="fe-icon-new"></i></a>' : '') +
						'</div>' +
						'<div class="desc desc-overflow">' +
							'<a href=\'javascript:urlform(\"' + element.rownum + '\", "post", "/board/view");\'>' + element.content + '</a>' +
						'</div>' +
						'<span class="writer">' + (element.type!=undefined ? '<img src="/img/lineage_writer.png" alt="">' : element.name + '<span class="server">' + serverName + '</span>') + '</span>&nbsp' +
					'</div>' +
...
function deleteConfirmAction(rownum){
	...
	urlform(rownum, 'post', '/board/delete' + getDeleteImgSrc(deleteImgList));
}
