$(function() {
	...
	listStyleBtn.on('click', function(event) {
		if (boardList.hasClass('board-list-card')) {
			viewMode('list');
			setLocal('board_content_viewMode', 'list');
		}
	});
	...
	cardStyleBtn.on('click', function(event) {
		if (boardList.hasClass('board-list-default')) {
			viewMode('card');
			setLocal('board_content_viewMode', 'card');
		}
	});
	
	if (getLocal('board_content_viewMode') == 'card') {
		viewMode('card');
	}
...
function reportBtn(obj){
	...
	//reportModalOpen($(obj), '컨텐츠공모_ID_' + $(obj).attr('data-boardid'));
	reportModalOpen($(obj), 'Contest_ID_' + $(obj).attr('data-boardid'));
}
...
function commentReportBtn(obj){
	...
	//reportModalOpen($(obj), '컨텐츠공모_ID_' + $(obj).attr('data-boardid') + '_코멘트_ID_' + $(obj).attr('data-commentid'));
	reportModalOpen($(obj), 'Contest_ID_' + $(obj).attr('data-boardid') + '_Comment_ID_' + $(obj).attr('data-commentid'));
}
...
function likeBtn(obj){
	...
	boardLikeBtn($(obj), '/define/contents/like', {id : $(obj).attr('data-id')});
}
...
function commentLikeBtn(obj){
	...
	boardLikeBtn($(obj), '/define/contents/commentLike', {id : $(obj).attr('data-commentid'), boardnum : $(obj).attr('data-boardnum')});
}
...
function boardCommentWrite(){
	...
	$.ajax ({
		type:"post",
		datatype:"json",
		url:"/define/contents/commentInsert",
		data:sendData,
		success:function(data){
			var commentHtml = 
...
function deleteCommentAction(commentId){
	$.ajax ({
		type:		"post",
		datatype:	"json",
		url:		"/define/contents/commentDelete",
		data:		{id : commentId},
		success:function(data){
			if(data){
...
function createBoardHtml(element){
	...
	return '<li class="board-items" onClick=\'urlform(\"' + element.rownum + '\", "post", "/contents/view");\'>' +
				'<div class="title board-type-list" style=\"' + listtype + '\">' +
					//'<span class="category">컨텐츠&nbsp;</span>' +
					'<span class="category">Content&nbsp;</span>' +
					'<a href=\'javascript:urlform(\"' + element.rownum + '\", "post", "/contents/view");\'>' + element.title + '</a>' +
				...
				'<div class="comment">' +
				'<a href=\'javascript:urlform(\"' + element.rownum + '\", "post", "/contents/view");\'>' +
				...
				(element.mainImg != undefined ? '<div class="board-items-thumb"><a href=\'javascript:urlform(\"' + element.rownum + '\", "post", "/contents/view");\'><img src=\"' + element.mainImg + '\" alt=""></a></div>' : '') +
					'<div class="board-items-contents">' +
						'<div class="title">' +
							//'<span class="category">컨텐츠&nbsp;</span>' +
							'<span class="category">Content&nbsp;</span>' +
							'<a href=\'javascript:urlform(\"' + element.rownum + '\", "post", "/contents/view");\'>' + element.title +
							(dateterm===0 ? '&nbsp<i class="fe-icon-new"></i></a>' : '') +
						'</div>' +
						'<div class="desc desc-overflow">' +
							'<a href=\'javascript:urlform(\"' + element.rownum + '\", "post", "/contents/view");\'>' + element.content + '</a>' +
						'</div>' +
						'<span class="writer">' + (element.type!=undefined ? '<img src="/img/lineage_writer.png" alt="">' : element.name + '<span class="server">' + serverName + '</span>') + '</span>&nbsp' +
					'</div>' +
...
function deleteConfirmAction(rownum){
	...
	urlform(rownum, 'post', '/contents/delete' + getDeleteImgSrc(deleteImgList));
}
...
function contentCancel(cancelType){
